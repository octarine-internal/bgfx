...
vs_%.bin.h : vs_%.sc
	@echo [$(<)]
	 $(SILENT) $(SHADERC) $(VS_FLAGS) --platform linux   -p 130         -f $(<) -o "$(SHADER_TMP)" --bin2c $(basename $(<))_glsl
	@cat "$(SHADER_TMP)" > $(@)
...
fs_%.bin.h : fs_%.sc
	@echo [$(<)]
	 $(SILENT) $(SHADERC) $(FS_FLAGS) --platform linux   -p 130         -f $(<) -o "$(SHADER_TMP)" --bin2c $(basename $(<))_glsl
	@cat "$(SHADER_TMP)" > $(@)
...
cs_%.bin.h : cs_%.sc
	...
	@cat "$(SHADER_TMP)" > $(@)
	-$(SILENT) $(SHADERC) $(CS_FLAGS) --platform android -p 300_es      -f $(<) -o "$(SHADER_TMP)" --bin2c $(basename $(<))_essl
	-@cat "$(SHADER_TMP)" >> $(@)	
